...
import { clear, h, icon, linkifyContact } from "@/dom.ts";
import {
  copyBase64,
  copyText,
  downloadFile,
...
import { getFieldTypeInfo, validateField } from "@/validators.ts";
...
export interface EditorRefs {
  ...
  previewCardsEl: HTMLElement;
  replaceBtn: HTMLButtonElement;
  replaceHintEl: HTMLElement;
}
...
function autocompleteFor(kind: string): string {
...
function isSafeHtmlPattern(pattern: string): boolean {
  try {
...
function buildInput(
...
): { input: HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement; kind: string } {
  ...
  if (kind === "textarea") {
    ...
    input = h("select", { id, autocomplete }) as HTMLSelectElement;
    const options = field.restrictions?.options || [];
    if (field.required === false) input.appendChild(h("option", { value: "" }, "—"));
    ...
    input = h("input", { type: "checkbox", id, autocomplete: "off" }) as HTMLInputElement;
    input.checked = Boolean(initialValue);
  } else if (kind === "number") {
    const attrs: Record<string, string> = { type: "number", id, autocomplete };
    if (field.restrictions?.min != null) attrs.min = String(field.restrictions.min);
    if (field.restrictions?.max != null) attrs.max = String(field.restrictions.max);
    input = h("input", attrs) as HTMLInputElement;
    input.value = String(initialValue ?? "");
  } else if (kind === "date" || kind === "datetime-local" || kind === "password") {
    input = h("input", { type: kind, id, autocomplete }) as HTMLInputElement;
    input.value = String(initialValue ?? "");
  } else {
    ...
    if (field.restrictions?.maxLength != null)
      attrs.maxlength = String(field.restrictions.maxLength);
    if (field.restrictions?.pattern && isSafeHtmlPattern(field.restrictions.pattern)) {
...
function fieldsUsedIn(content: string, fields: TemplateField[]): TemplateField[] {
  return fields.filter((f) => content.includes(`__${f.key}__`));
}

async function confirmIfIncomplete(invalidFields: TemplateField[]): Promise<boolean> {
...
function buildFieldRow(
  ...
  const changeEvent = kind === "select" || kind === "checkbox" ? "change" : "input";
  input.addEventListener(changeEvent, () => {
    onChange(kind === "checkbox" ? (input as HTMLInputElement).checked : input.value);
  });
  ...
  const rowChildren: (HTMLElement | null)[] = [label];
  if (field.description) rowChildren.push(h("p", { class: "field-help" }, field.description));
...
export function mountEditor(refs: EditorRefs, template: Template): void {
  const fields = getDedupedFields(template);
  const values: FieldValues = initialValuesFor(fields);
  const errors: Record<string, string | null> = {};
  const previewEls: Record<string, HTMLElement> = {};
  const fieldRows: Record<string, BuiltRow> = {};

  refs.titleEl.textContent = template.name || "(unnamed template)";
  clear(refs.metaEl);
  if (template.version) refs.metaEl.appendChild(document.createTextNode(`v${template.version}`));
  if (template.author) {
    if (template.version) refs.metaEl.appendChild(document.createTextNode(" · "));
  ...
  function invalidFieldsFor(sub: SubTemplate): TemplateField[] {
    return fieldsUsedIn(sub.content, fields).filter((f) => f.required !== false && errors[f.key]);
  }

  function handleFieldChange(field: TemplateField, rawValue: string | boolean) {
  ...
  if (!fields.length) {
    refs.formEl.appendChild(
      h("p", { class: "empty-state" }, "This template has no configurable fields."),
    );
  }

  for (const field of fields) {
    const built = buildFieldRow(field, values[field.key], (val) => handleFieldChange(field, val));
    fieldRows[field.key] = built;
    refs.formEl.appendChild(built.row);
    const actual =
      built.kind === "checkbox" ? (built.input as HTMLInputElement).checked : built.input.value;
  ...
  refs.replaceBtn.onclick = async () => {
    const invalidRequired = fields.filter((f) => f.required !== false && errors[f.key]);
    ...
    try {
      ...
      refs.replaceHintEl.textContent = (err as Error).message;
    }
  };
}
