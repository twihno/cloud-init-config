...
export async function copyBase64(text: string): Promise<void> {
...
export interface ReplaceOnDiskResult {
  cancelled: boolean;
  written: string[];
...
export async function replaceOnDisk(files: FileToWrite[]): Promise<ReplaceOnDiskResult> {
  ...
  const hasClassicFile = CLASSIC_CLOUD_INIT_FILES.some((f) => existingNames.includes(f));
  const hasPiHint = RASPBERRY_PI_BOOT_HINTS.some((f) => existingNames.includes(f));

  if (!hasClassicFile) {
    const proceed = await confirmDialog({
      title: hasPiHint ? "Empty Raspberry Pi boot partition?" : "Unrecognized folder",
      message: hasPiHint
