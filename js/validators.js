...
function isValidCIDR(value, addrValidator, maxPrefix) {
...
function isValidTimeParts(h, min, s) {
  return h >= 0 && h <= 23 && min >= 0 && min <= 59 && s >= 0 && s <= 60; // 60 = leap second
}

function isValidDatetimeLocal(str) {
...
function isValidBase64String(str) {
  return (
    str.length > 0 && str.length % 4 === 0 && /^[A-Za-z0-9+/]+={0,2}$/.test(str)
  );
}

function isValidSSHPublicKeyLine(line) {
...
function checkLength(value, restrictions) {
...
function checkPattern(value, restrictions) {
  if (restrictions.pattern) {
    ...
    if (!re.test(value)) {
      return (
        restrictions.patternMessage ||
        `Must match pattern: ${restrictions.pattern}`
      );
    }
  }
  return null;
...
function stringValidator(value, restrictions) {
  return (
    checkLength(value, restrictions) ||
    checkPattern(value, restrictions) || { valid: true }
  );
}
...
export const VALIDATORS = {
  ...
  int: {
    ...
    validate: (v, r) => {
      ...
      if (r.min != null && n < r.min)
        return { valid: false, message: `Must be ≥ ${r.min}.` };
      if (r.max != null && n > r.max)
        return { valid: false, message: `Must be ≤ ${r.max}.` };
      return { valid: true };
    },
  },
  float: {
  ...
  enum: {
    ...
    validate: (v, r) => {
      ...
      if (!options.includes(v))
        return { valid: false, message: "Not one of the allowed options." };
      return { valid: true };
    },
  },
  ipv4: {
    ...
    validate: (v) =>
      isValidIPv4(v)
        ? { valid: true }
        : { valid: false, message: "Not a valid IPv4 address." },
  },
  ipv6: {
    label: "IPv6 address",
    inputKind: "text",
    ...
    validate: (v) =>
      isValidIPv6(v)
        ? { valid: true }
        : { valid: false, message: "Not a valid IPv6 address." },
  },
  cidr4: {
    label: "IPv4 CIDR",
    inputKind: "text",
  ...
  fqdn: {
    ...
    validate: (v) => {
      if (v.length > 253)
        return { valid: false, message: "Too long for a domain name." };
      ...
      return ok
        ? { valid: true }
        : { valid: false, message: "Contains an invalid domain label." };
    },
  },
  email: {
    label: "Email address",
  ...
  url: {
    ...
    validate: (v, r) => {
      ...
      if (
        r.protocols &&
        !r.protocols.includes(parsed.protocol.replace(":", ""))
      ) {
        return {
          valid: false,
    ...
    validate: (v) =>
      isValidRFC3339(v)
        ...
        : {
            valid: false,
            message:
              "Not a valid RFC 3339 timestamp (e.g. 2026-09-15T12:00:00Z).",
          },
  },
  "unix-timestamp": {
    ...
    validate: (v) =>
      isValidSSHPublicKeyLine(v)
        ...
        : {
            valid: false,
            message:
              "Not a recognized SSH public key (ssh-rsa, ssh-ed25519, ecdsa-sha2-*, …).",
          },
  },
  "ssh-pubkey-list": {
...
export function validateField(field, rawValue) {
  ...
  if (!type)
    return { valid: false, message: `Unknown field type "${field.type}".` };
  ...
  const result = type.validate(value, field.restrictions || {});
  return result.valid
